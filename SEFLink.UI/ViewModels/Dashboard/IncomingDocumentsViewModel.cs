...
{
    public class IncomingDocumentsViewModel : Observable
    {
        #region Fields
        ...
        public IncomingDocumentsViewModel(IDocumentInfoDataService documentInfoDataService, IEventAggregator eventAggregator)
        {
        ...
        private void LoadViewData()
        {
            ...
            TotalNumberOfDocuments = DocumentInfoDataService.GetIncomingCount();
        ...
        public async void ShowDocuments()
        {
            ...
            var docs = await DocumentInfoDataService.GetAllIncomingDocumentsAsync();
            ...
            {                
                foreach (var Doc in ShownDocuments)
                {
                    if (Doc.IsSelected)
                    {
                        ...
                        if (DocumentInfoDataService.DeleteIncomingById(Id))
                        {
            ...
            if (SelectedAction == Actions[2])      // "Arhiviraj"
            {
                foreach (var Doc in ShownDocuments)
                {
                    if (Doc.IsSelected)
                    {
                        ...
                        DocumentInfoDataService.GetIncomingDocumentById(Id).Status = DocStatus.Archived;
                    }
                }
            }
